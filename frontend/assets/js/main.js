        ...
        $('.js-sections').pagepiling({
            ...
            afterRender: function afterRender() {
                ...
                for (var i = 0; i < sectionCount; i++) {
                    $('<div/>', {"class": 'section-scroll-item'}).appendTo('.js-section-scroll');
                }
                $(".section-scroll-item").eq(0).addClass("active");
                $('.js-section-scroll').css({height: $(window).innerHeight() / 2});
                $('.section-scroll-item').css({height: $('.js-section-scroll').innerHeight() / $('.section-scroll-item').length});
            }
        });
    }
}
...
function imageMoving() {
    ...
    $(".js-projects-img").on("mousemove", function (e) {
        ...
        if (e.pageX < elem_left || e.pageX > elem_left + $(this).innerWidth() || e.pageY < elem_top || e.pageY > elem_top + $(this).innerHeight()) {
            $(this).css("opacity", "0");
            $(this).find(".projects-list__img").css({'transform': 'translateY(0) translateX(0)'});
        } else {
            $(this).css("opacity", "1");
            var Xinner = e.pageX - elem_left - $(this).innerWidth() / 2 + 40;
            var Yinner = e.pageY - elem_top - $(this).innerHeight() / 2 + 40;
            $(this).find(".projects-list__img").css({'transform': 'translateY(' + Yinner + 'px) translateX(' + Xinner + 'px)'});
        }
    });
}
...
function sendForm() {
    if ($('.js-form').length) {
        $('.js-form').each(function () {
            $(this).validate({
                errorClass: 'error wobble-error', submitHandler: function submitHandler(form) {
                    $.ajax({
                        type: "POST", url: "mail.php", data: $(form).serialize(), success: function success() {
                            $('#error').modal('hide');
                            $('#success').modal('show');
...
function heroParalax() {
    if ($('.js-hero').length) {
        $(window).on('scroll', function () {
            $('.js-hero').css({'transform': 'translateY(' + $(window).scrollTop() * 0.3 + 'px)'});
        });
    }
}
